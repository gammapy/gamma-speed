...
def monitor_plot(infile, nfiles, outpref, func):
    # read the values into a csv file        
    fig = plt.figure(1, figsize=(15.0, 15.0))
    for i in [0]:
        
        filename = infile + "_CPUs=" + str(i + 1) + ".csv"
...
#         plt.title('IO write counts for ' + name)     
    ...
    if func == 'ctobssim':    
        ...
        plt_name = 'plots/' + outpref + '_' + func + '_' + platform.node() + '.png'
        fig.suptitle('Machine: ' + platform.node())
...
def main():
    ...
    monitor_plot(infile=args.infile, nfiles=int(args.nrcsv), outpref=args.outfile, func=args.function)
