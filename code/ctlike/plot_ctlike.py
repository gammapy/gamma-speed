...
def ctlike_separate_plots(my_plotter, ncsv, outpref, log_exists=False):
    ...
    io_cumul_plot = my_plotter.IO_cumulative_plot('plots/'+outpref+'io_write_cumul')
    io_speed_plot = my_plotter.IO_speed_plot('plots/'+outpref+'io_write')
...
def ctlike_speed_up(my_plotter, outpref, log_exists=False):
    ...
    if log_exists:
        ...
        for i in xrange(my_plotter.ncsv):
            ...
            aux = select_lines('ctlike_CPUs=' + str(i + 1) + '.csv', df.at[0, 'TIME'], ['gammaspeed:real_parallel_start', 'gammaspeed:real_parallel_end'])
        ...
        plt.figure(21)
        my_plotter.splot(figtitle = 'Overall speed up and efficiency for ctlike', outfile="plots/" +outpref+'_general', speed_frame = times, amdahl_frame = amd)
             
        plt.figure(20)
        my_plotter.splot(figtitle = 'Parallel speed up and efficiency for ctlike', outfile="plots/" +outpref+'_parallel', speed_frame = parallel_loop, amdahl_frame = pd.Series(data=cores, index=cores))
